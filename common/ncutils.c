...
void ncu_readfield_double(char fname[], char varname[], int k, int ni, int nj, int nk, double* v)
{
    ...
    if (hasrecorddim)
        if (dimlen[0] == 0)
            quit("ncu_readfield_double(): \"%s\": %s: empty record dimension", fname, varname);

    if (ndims == 4) {
        if (nj == 0) {
            quit("ncu_readfield_double(): \"%s\": %s: expected positive \"j\" dimension for a 4-dimensional variable\n", fname, varname);
        }
        if (!hasrecorddim && dimlen[0] != 1)
            quit("ncu_readfield(): \"%s\": %s: for a 4-dimensional variable expected the first dimension to be either unlimited or of length 1\n", fname, varname);
        start[0] = dimlen[0] - 1;
        if (dimlen[1] != nk) {
            if (dimlen[1] != 1)
                quit("ncu_readfield_double(): \"%s\": vertical dimension of variable \"%s\" (nk = %d) does not match grid dimension (nk = %d)", fname, varname, dimlen[1], nk);
            else
                k = 0;          /* ignore k */
        }
        if ((ni >= 0 && nj >= 0) && (dimlen[3] != ni || dimlen[2] != nj))
            quit("ncu_readfield_double(): \"%s\": horizontal dimensions of variable \"%s\" (ni = %d, nj = %d) do not match grid dimensions (ni = %d, nj = %d)", fname, varname, dimlen[3], dimlen[2], ni, nj);
        start[1] = k;
        start[2] = 0;
        start[3] = 0;
        count[0] = 1;
        ...
        if (nj > 0) {
            if (!hasrecorddim) {
                if (nk >= 0 && dimlen[0] != nk && !(dimlen[0] == 1 && (k == 0 || k == nk - 1)))
                    quit("ncu_readfield_double(): \"%s\": vertical dimension of variable \"%s\" (nk = %d) does not match grid dimension (nk = %d)", fname, varname, dimlen[0], nk);
                start[0] = (dimlen[0] == 1) ? 0 : k;
                start[1] = 0;
                start[2] = 0;
                count[0] = 1;
            ...
            if ((ni >= 0 && nj >= 0) && (dimlen[2] != ni || dimlen[1] != nj))
                quit("ncu_readfield_double(): \"%s\": horizontal dimensions of variable \"%s\" (ni = %d, nj = %d) do not match grid dimensions (ni = %d, nj = %d)", fname, varname, dimlen[2], dimlen[1], ni, nj);
        } else {
            if (!hasrecorddim && dimlen[0] != 1)
                quit("ncu_readfield_double(): \"%s\": %s: for a 3-dimensional variable on unstructured horizontal grid expected the first dimension to be either unlimited or of length 1\n", fname, varname);
            start[0] = dimlen[0] - 1;
            if (nk >= 0 && dimlen[1] != nk) {
                if (dimlen[1] != 1)
                    quit("ncu_readfield_double(): \"%s\": vertical dimension of variable \"%s\" (nk = %d) does not match grid dimension (nk = %d)", fname, varname, dimlen[1], nk);
            ...
            if (ni >= 0 && dimlen[2] != ni)
                quit("ncu_readfield_double(): \"%s\": horizontal dimension of variable \"%s\" (ni = %d) does not match grid dimension (ni = %d)", fname, varname, dimlen[2], ni);
            start[1] = k;
            start[2] = 0;
            count[0] = 1;
            count[1] = 1;
        ...
        if (nj > 0) {
            if (hasrecorddim)
                quit("ncu_readfield_double(): %s: can not read layer from a 1D variable \"%s\"", fname, varname);
            /*
             * ignore k
             */
            start[0] = 0;
            ...
            if (ni >= 0 && dimlen[1] != ni)
                quit("ncu_readfield_double(): \"%s\": horizontal dimension of variable \"%s\" (ni = %d) does not match grid dimension (ni = %d)", fname, varname, dimlen[1], ni);
        }
    } else if (ndims == 1) {
        if (nj > 0) {
            quit("ncu_readfield_double(): %s: can not read 2D field for \"%s\": # of dimensions = %d", fname, varname, ndims);
        } else {
            if (hasrecorddim)
                quit("ncu_readfield_double(): %s: can not read layer from a 0D variable \"%s\"", fname, varname);
            /*
             * ignore k in this case
             */
            start[0] = 0;
    ...
    ncw_get_vara_double(ncid, varid, start, count, v);
